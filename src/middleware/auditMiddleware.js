...
const routeMap = [
  ...
  { method: 'PATCH',  pattern: /\/purchase-orders\/.*\/submit/,    action: 'SUBMIT_PO',            module: 'procurement' },
  { method: 'PATCH',  pattern: /\/purchase-orders\/.*\/cancel/,    action: 'CANCEL_PO',            module: 'procurement' },
  { method: 'PUT',    pattern: /\/purchase-orders\//,              action: 'UPDATE_PURCHASE_ORDER', module: 'procurement' },
  { method: 'PATCH',  pattern: /\/purchase-orders\/.*\/send/,      action: 'SEND_PO',                module: 'procurement' },
  { method: 'PATCH',  pattern: /\/purchase-orders\/.*\/pay/,       action: 'PAY_PO',                 module: 'procurement' },
...
const descriptions = {
  ...
  UPDATE_PURCHASE_ORDER:(req) => `${req.user?.name} updated a draft purchase order`,
  SUBMIT_PO:            (req) => `${req.user?.name} submitted a purchase order for approval`,
  APPROVE_PO:           (req) => `${req.user?.name} approved a purchase order`,
  CANCEL_PO:            (req) => `${req.user?.name} cancelled a purchase order`,
  SEND_PO:              (req) => `${req.user?.name} sent a purchase order to supplier`,
  PAY_PO:               (req) => `${req.user?.name} recorded payment on a purchase order`,
  ...
  UPDATE_EMPLOYEE:      (req) => `${req.user?.name} updated an employee record`,
  TERMINATE_EMPLOYEE:   (req) => `${req.user?.name} terminated an employee`,
  RUN_BULK_PAYROLL:     (req) => `${req.user?.name} ran bulk payroll`,
